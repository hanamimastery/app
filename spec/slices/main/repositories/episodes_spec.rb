...
RSpec.describe Main::Repositories::Episodes do
  describe "#create_from_github" do
    ...
    it 'creates multiple records with correct transformation' do
      expect{ subject.create_from_github(tuples) }.to change { subject.episodes.count }.by(2)
    ...
    it 'creates single record' do
      expect { subject.create_from_github(tuples[1]) }.to change { subject.episodes.count }.by(1)
